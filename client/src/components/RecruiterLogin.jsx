...
const RecruiterLogin = () => {
   ...
   const {setShowRecruiterLogin} =useContext(AppContext)
    ...
    <div className=' absolute top-0 left-0 right-0 bottom-0 z-10 backdrop-blur-sm  bg-black/30 flex justify-center items-center '>
      <form onSubmit={onSubmitHandler} action="" className='relative bg-white p-10 rounded-xl font-medium'>
         <h1 className='text-center text-2xl text-neutral-700 font-medium'>Recruiter <span className='text-blue-600'>{state}</span></h1>
        ...
        <div className='border px-4 py-2 flex items-center gap-2 rounded-full mt-5'>
         ...
         <button type='submit' className=' bg-blue-600 w-full text-white py-2 rounded-full '>{state==="Login"?"Login":isTextDataSubmitted?"Create an Account":"next"}</button>
        {
            state==="Login"?
